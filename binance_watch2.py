...
def chk_signal(df):
    ...
    if df.iloc[pos-2]["close"] > df.iloc[pos-2]["EMA_5"]  and df.iloc[pos-2]["close"] > df.iloc[pos-2]["open"] and df.iloc[pos-2]["MACD_9_12_26"] >0 and  df.iloc[pos-2]["close"] < df.iloc[pos-2]["BBU_21_2.0"] and df.iloc[pos-1]["MACD_9_12_26"] > df.iloc[pos-2]["MACD_9_12_26"]: 
        return 'BUY'
    elif(df.iloc[pos-2]["close"] < df.iloc[pos-2]["EMA_5"]  or df.iloc[pos-1]["MACD_9_12_26"] < df.iloc[pos-2]["MACD_9_12_26"])and got_order: 
        return 'SELL'
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if sign == 'BUY' and not got_order:
            ...
            lineNotify.send_alert('BNBUSDT ++BUY++ at {}'.format(buy_value))
            with open('price','wb') as f:
                f.write('BNBUSDT ++BUY++ at {}'.format(buy_value))
            ...
            print(f" [{profitlst[-1]}%]")
            lineNotify.send_alert('BNBUSDT --SELL-- at {} and take profit {}'.format(sell_value,sell_value-buy_value))
            with open('price','wb') as f:
                f.write('BNBUSDT --SELL-- at {} and take profit {}'.format(sell_value,sell_value-buy_value))
