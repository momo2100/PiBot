...
if __name__ == "__main__":
    ...
    if sign != fsign:
        if sign == 'BUY':
            lineNotify.send_pic(symbol,'BNBUSDT ++BUY++ at {}'.format(client.get_ticker(symbol=symbol)['bidPrice']))
            with open('Notics.txt','w') as f:
                f.write("BUY")
        elif sign == 'SELL':
            lineNotify.send_pic(symbol,'BNBUSDT --SELL-- at {}'.format(client.get_ticker(symbol=symbol)['bidPrice']))
            with open('Notics.txt','w') as f:
                f.write("SELL")
